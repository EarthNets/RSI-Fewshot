...
You need to create a new directory named `rsifewshot/detection/core/optimizer`.
And then implement the new optimizer in a file, e.g., in `rsifewshot/detection/core/optimizer/my_optimizer.py`:
...
- Modify `rsifewshot/detection/core/optimizer/__init__.py` to import it.

  The newly defined module should be imported in `rsifewshot/detection/core/optimizer/__init__.py` so that the registry will
...
```python
custom_imports = dict(imports=['rsifewshot.detection.core.optimizer.my_optimizer'], allow_failed_imports=False)
```

The module `rsifewshot.detection.core.optimizer.my_optimizer` will be imported at the beginning of the program and the class `MyOptimizer` is then automatically registered.
Note that only the package containing the class `MyOptimizer` should be imported.
`rsifewshot.detection.core.optimizer.my_optimizer.MyOptimizer` **cannot** be imported directly.
...
Then we need to make `MyHook` imported. Assuming the file is in `rsifewshot/detection/core/utils/my_hook.py` there are two ways to do that:

- Modify `rsifewshot/core/utils/__init__.py` to import it.

  The newly defined module should be imported in `rsifewshot/detection/core/utils/__init__.py` so that the registry will
  find the new module and add it:
...
```python
custom_imports = dict(imports=['rsifewshot.detection.core.utils.my_hook'], allow_failed_imports=False)
```
