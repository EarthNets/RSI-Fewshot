...
- [rsidet](https://rsidetection.readthedocs.io/en/latest/#installation) 2.16.0+
...
2. 安装 MMClassification 和 MMDetection.
   ...
   你可以直接通过如下命令从 pip 安装使用 mmclassification 和 rsidetection：
...
3. 安装 MMFewShot.
   ...
   你可以直接通过如下命令从 pip 安装使用 rsifewshot：
...
我们提供了 [Dockerfile](https://github.com/open-mmlab/rsifewshot/blob/master/docker/Dockerfile) to build an image. Ensure that you are using [docker version](https://docs.docker.com/engine/install/) >=19.03.
...
具体的细节可以参考 [few shot classification demo](https://github.com/open-mmlab/rsifewshot/tree/main/demo#few-shot-classification-demo)
以及 [few shot detection demo](https://github.com/open-mmlab/rsifewshot/tree/main/demo#few-shot-detection-demo) 。
如果成功安装 MMFewShot，则上面的代码可以完整地运行。

## 准备数据集

具体的细节可以参考 [准备数据](https://github.com/open-mmlab/rsifewshot/tree/main/tools/data) 下载并组织数据集。
